<div class="w-full lg:w-2/3 lg:ml-4 mt-4 lg:mt-0">
    @if($selectedMatch)  
        <div class="flex flex-col bg-white lg:h-full h-[650px]">
            <div class="flex items-center px-6 py-2 border-b border-black bg-white">
                <div class="flex items-start">
                    <a href="{{ route('user.chat.index', $selectedMatch->id) }}" class="hover:text-blue-100">
                ...
                <div class="flex flex-1 items-center justify-center gap-4">
                    @if ($selectedMatch->animal->organization->image)
                        <img src="{{ asset('storage/'.$selectedMatch->animal->organization->image->path) }}" alt="画像"
                    ...
                    <div class="flex flex-col items-center">
                        <p class="text-lg font-semibold">
                            {{ $selectedMatch->animal->organization->org_name }}
                        </p>
                        <p class="text-sm text-gray-500">
                            {{ $selectedMatch->animal->animal_name }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-black">
                <form action="{{ route('user.chat.store', $selectedMatch) }}" method="POST" class="flex items-center gap-2 mt-5">
                    @csrf
