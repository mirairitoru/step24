...
<div class="w-full lg:w-1/3 border-b lg:border-b-0 lg:border-r border-black">
    <div class="px-4">
        <div class="flex flex-col sm:flex-row gap-4 mb-4">
            <button class="border hover:text-white hover:bg-blue-500 px-2 py-2 text-sm rounded-md
                {{ request()->routeIs('org.favorite.index') ? 'bg-blue-500 text-white' : 'bg-blue-200' }}"
                onclick="window.location.href='{{ route('org.favorite.index') }}'">
                興味あり一覧
            </button>
            <button class="border bg-blue-200 hover:text-white hover:bg-blue-500 px-2 py-2 text-sm rounded-md"
    ...
    <div class="max-h-[350px] lg:max-h-[650px] overflow-y-auto px-4">
        @foreach ($animals as $animal)
            <a href="{{ route('org.favorite.index', [
                ...
                class="w-full flex items-center gap-3 p-2 mb-4 border rounded relative hover:bg-orange-100 hover:border-orange-300 cursor-pointer
                ...
                <span class="bg-blue-500 text-white text-xs px-2 py-1 rounded absolute top-0 right-0">
